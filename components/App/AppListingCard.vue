<template>
  <div class="AppListingCard">
    ...
    >
      <font-awesome-icon class="AppListingCard__star" :icon="iconItem" />
    ...
    <img :src="item.image" class="AppListingCard__image" alt="Property Image" />
  ...
  computed: {
    infoList() {
      let price = this.item.price
        ? `$ ${this.item.price.toString().replace(/(\d)(?=(\d{3})+$)/g, "$1,")}`
        : "";
      price = this.item.type === "RNT" ? `${price}/mo` : price;
      const list = [
        {
          name: "Bed",
          value: this.item.bed,
