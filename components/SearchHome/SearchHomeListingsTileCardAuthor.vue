<template>
  <div
    class="SearchHomeListingsTileCardAuthor"
    :class="{
      'SearchHomeListingsTileCardAuthor--active' : isActiveOnMap
    }"
    @click="selectItem($event, item)"
  >
    <div class="SearchHomeListingsTileCardAuthor__img">
      <img
        v-if="item.photos[0]"
        :src="item.photos[0]"
        alt="image"
    ...
    <div class="SearchHomeListingsTileCardAuthor__info">
      <!-- <h2
        v-if="item.property.lotDescription"
        class="SearchHomeListingsTileCardAuthor__description"
      >
      ...
      <h1
        ...
        class="SearchHomeListingsTileCardAuthor__title"
      >
        {{ item.address.full }}
      </h1>
      ...
      <p
        ...
        class="SearchHomeListingsTileCardAuthor__address"
      >
        {{ item.address.city }}, {{ item.address.state }}, {{ item.address.postalCode }}
      </p>

      <div class="SearchHomeListingsTileCardAuthor__details">
        <div
          v-for="(detail, i) in infoList"
          :key="i"
          class="SearchHomeListingsTileCardAuthor__detail"
        >
          <p
            class="SearchHomeListingsTileCardAuthor__detail-name"
            v-html="detail.name"
          />

          <p
            class="SearchHomeListingsTileCardAuthor__detail-value"
            v-html="detail.value"
          />
        </div>
      </div>

      <div class="SearchHomeListingsTileCardAuthor__action">
        <AppLinkButton
          text="View Listing"
          color="pink"
          type="small"
        ...
        <AppLink
          :text="'explore more in ' + item.address.city.toLowerCase()"
          color="grey"
          class="SearchHomeListingsTileCardAuthor__explore"
...
export default {
  ...
  computed: {
    infoList () {
      let price = this.item.listPrice ? `$ ${this.item.listPrice.toString().replace(/(\d)(?=(\d{3})+$)/g, '$1,')}` : '';
      price = this.item.property.type === "RNT" ? `${price}/mo` : price;
      const list = [
        {
          name: 'Bed',
  ...
  methods: {
    ...
    selectItem (event, value) {
      if (this.showingType === "Map") {
        this.$emit('findMap', value.geo, this.index)
      } else {
        this.$router.push(this.getLinkUrl())
      }
    },
