<template>
  <div>
    <LookingSellMain :main="sell.main_sliders" />

    <LookingSellWorks :works="sell.content_data[0]" />

    <LookingSellProactive :proactive="sell.content_data[1]" />

    <LookingSellAgent :agent="sell.content_data[2]" />

    <!-- <LookingSellCover
      :cover="sell.content_data[3]"
    /> -->

    <LookingSellRent :rent="sell.content_data[4]" />

    <LookingSellProof :proof="sell.content_data[5]" />

    <AppContactForm :form="sell.form_data" :content="sell.content_data[6]" />
  </div>
</template>
...
export default {
  async asyncData({ store }) {
    ...
    await store.dispatch("gravity-form/getGravityForm", 14);
  },
